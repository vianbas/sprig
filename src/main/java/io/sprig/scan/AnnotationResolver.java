 ...
 *
 * <p>We deliberately never call symbol resolution ({@code expr.resolve()}): it needs the project's
 * compiled classpath, which a static linter does not have. Instead, the FQN is derived from the
 * import table:
 *
 * <ol>
 *   <li>already fully-qualified names are matched directly;
 *   <li>an explicit import whose simple name equals the annotation wins;
 *   <li>a wildcard import is accepted only when it names a known Spring package.
 * </ol>
 *
 * Code that uses an annotation without importing it would not compile in a real project, so
 * skipping those is sound and kills most false positives.
 */
public final class AnnotationResolver {

    /** Simple annotation name → candidate fully-qualified names. */
    private static final Map<String, List<String>> KNOWN_FQNS =
            Map.ofEntries(
                    Map.entry(
                            "CrossOrigin",
                            List.of("org.springframework.web.bind.annotation.CrossOrigin")),
                    Map.entry(
                            "EnableWebSecurity",
                            List.of(
                    ...
                    Map.entry(
                            "Configuration",
                            List.of("org.springframework.context.annotation.Configuration")),
    ...
    /**
     * Reads a boolean-valued member; {@code "true"}/{@code true} → {@code true}, absent → {@code
     * false}.
     */
            ...
            if (pair.getNameAsString().equals(name)) {
                ...
                if (value instanceof ArrayInitializerExpr array) {
                    return array.getValues().stream()
                            .map(AnnotationResolver::literalToString)
                            .toList();
                }
