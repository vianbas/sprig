        ...
        assertThat(files.javaFiles())
                .anyMatch(
                        p ->
                                p.toString()
                                        .endsWith(
                                                "cors-misconfig/src/main/java/demo/CorsController.java"));
        assertThat(files.javaFiles())
                .anyMatch(
                        p ->
                                p.toString()
                                        .endsWith(
                                                "secure-app/src/main/java/demo/SecurityConfig.java"));
    }

    /**
     * The scan root is not always absolute — the CLI's default target is the relative path "." — so
     * exclude-path matching must be independent of how the caller specified {@code root}.
     * Regression test for a bug where the pattern was matched against the raw walked path (which
     * carries whatever prefix {@code root} was given), so a natural-looking pattern like
     * "src/test/resources/fixtures/**" silently excluded nothing at all when root was "." instead
     * of an absolute path.
     */
    @Test
